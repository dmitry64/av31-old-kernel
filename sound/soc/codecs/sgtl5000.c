...
static int sgtl5000_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
        ...
        printk(KERN_INFO "sglt5000_brobe: enter\n");
	...
	if (ret < 0) {
		dev_err(codec->dev, "failed to create pcms\n");
                printk(KERN_INFO "sglt5000_brobe: failed to create pcms\n");
		return ret;
	}
					...
					const struct i2c_device_id *id)
{
	...
	u32 val;
        ...
        printk(KERN_INFO "sglt5000_i2c_brobe: enter\n");
	...
	codec->dev = &client->dev;
        ...
        printk(KERN_INFO "sglt5000_i2c_brobe: &client->dev = %x\n",(unsigned int)&client->dev);
	...
	if (ret != 0) 
		...
		dev_err(codec->dev, "Failed to register codec: %d\n", ret);
                printk(KERN_INFO "sglt5000_i2c_brobe: failed to register codec = %d\n",ret);
		goto err_codec_reg;
	}

        printk(KERN_INFO "sglt5000_i2c_brobe: &dai = %x, >dev = %x\n",(unsigned int)&sgtl5000_dai,sgtl5000_dai.dev);
	ret = snd_soc_register_dai(&sgtl5000_dai);
	if (ret != 0) 
        {
		dev_err(codec->dev, "Failed to register DAIs: %d\n", ret);
                printk(KERN_INFO "sglt5000_i2c_brobe: failed to register DAI = %d\n",ret);
		goto err_codec_reg;
	}
